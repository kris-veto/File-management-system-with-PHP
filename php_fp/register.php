...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $phone = $_POST['phone'];
        ...
        if (!preg_match('/^[0-9]{10}$/', $phone)) {  // Validation of phone number
    ...
    $password = $_POST['password'];
        ...
        if (!preg_match('/^(?=.*[a-z])(?=.*\d)[a-zA-Z\d]{6,}$/', $password)) {
            echo "Error: Invalid password. Password must be at least 6 alphanumeric characters long including at least one digit.";
            exit();
        }
    // Hash the password
    ...
    $stmt = $conn->prepare(
        "INSERT INTO r_users (
            username, phone, birthdate, email, password
            ) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sisss", $username, $phone, $birthdate, $email, $hashed_password); /*$hashed_password*/
    if ($stmt->execute()) {
